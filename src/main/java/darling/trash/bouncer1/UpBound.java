...
public class UpBound {
    ...
    public void update(HistoricCandle lastNormalBar, Atr5 atr5) {
        if (point == null) {
            point = new HistoricPoint(lastNormalBar.time(), lastNormalBar.high(), lastNormalBar.volume());
            return;
        }
        ...
        if (delta.compareTo(point.price().multiply(new BigDecimal("0.0005"))) <= 0) {
            point = new HistoricPoint(point.time(), point.price().add(delta), lastNormalBar.volume());
            holdCount = holdCount >= 2 ? holdCount : holdCount + 1;
            lessCount = 0;
        } else {
            point = new HistoricPoint(lastNormalBar.time(), point.price().add(delta), lastNormalBar.volume());
            lessCount = 0;
            holdCount = 1;
        }
    }
    ...
    public BigDecimal calcDelta(HistoricCandle lastNormalBar, Atr5 atr5) {
        BigDecimal delta = lastNormalBar.high().subtract(point.price());
        BigDecimal atr07Length = atr5.get().multiply(new BigDecimal("0.7"));
        BigDecimal atr15Length = atr5.get().multiply(new BigDecimal("1.5"));
